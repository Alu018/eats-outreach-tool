  ...
  try {
    ...
    const reps: Rep[] = rows.slice(1).map((r, index) => {
      return {
        ...
        signed118th: r[4]?.toLowerCase() === 'y',
        signed117th: r[5]?.toLowerCase() === 'y',
        signed115th: r[6]?.toLowerCase() === 'y',
        legislativeContacts: r[7] || '',
        chiefOfStaff: r[8] || '',
        headOfComms: r[9] || '',
        legislativeDirector: r[10] || '',
        cbc: r[11] || '',
        hispanicCaucus: r[12] || '',
        officePhone: r[13] || '',
        senatorsSignedSenateVersion: r[14] || '',
      }
    })
